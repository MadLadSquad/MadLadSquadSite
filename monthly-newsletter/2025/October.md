...
1. CMake improvements:
   ...
   - Split platform-dependent code into platform-specific backends
   - Info.plist files used when exporting as macOS application bundles are now generated by CMake
1. Packaged the framework and our applications as deb packages. More information at: <https://debs.madladsquad.com>
...
1. Fixed a weird OpenGL crash on Wayland
1. Added additional RendererUtils functions for window and renderer integration
1. Added a new WindowUtils interface for integrating with the underlying windowing system's events API
...
1. Working on better testing workflows
...
sleep app (not really imaginative, but it gets the point across). The idea is to create mobile and desktop applications that are installed on
all of the user's personal devices. This app will then keep track of whether you're online on any of your devices before or after a bedtime
goal you have set. The idea of the app is to make it easy for a person to gradually transition away from a night-owl lifestyle, which
