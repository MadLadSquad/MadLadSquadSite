...
## DISCLAIMER
Any applications and comments on their UI here are for pure example reasons; we do not want to criticise the applications for
...
## Design policies
1. Keep the UI simple and intuitive; if there are a lot of options, try hiding them in some way, such as in a ribbon menu
...
1. Prefer to automate user actions as much as possible
...
1. When developing an application, think of its usage. Some applications may require the usage of certain terminology
1. When marking up text, try to stick to proven conventions: bold for important information, etc.
1. Try not to hardcode text, as this might not be helpful to users with different visibility problems
...
1. Keep text at a normal readable size, not too small but not too big. Strive to make text a little bit larger than a standard book
...
### Layout
1. We generally don't restrict minimum sizes for most applications, though if your application is not specialised, use 800x600
